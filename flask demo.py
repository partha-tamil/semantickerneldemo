...
def home():
    """
    A simple GET route to confirm the application is running.
    """
    return "Hello from your Flask app on Azure Container Apps! Send a POST request to /process-data."

@app.route('/process-data', methods=['POST'])
def process_data():
    """
    Handles POST requests, expecting JSON data in the request body.
    It extracts 'name' and 'message' from the JSON and returns a custom response.
    """
    if request.is_json:
        try:
            data = request.get_json()
            workitem = data.get('workitem')
            asyncio.run(automated_devops_workflow(workitem))
            return jsonify({"status": "success"}), 200
        except Exception as e:
            # Handle cases where JSON is malformed or other processing errors occur
            return jsonify({"status": "error", "message": f"Invalid JSON or processing error: {str(e)}"}), 400
    else:
...
def get_id_by_name(target_name):
    ...
    """
    json_data = list_pipelines(azure_devops_url, azure_devops_pat, azure_devops_project)
...
def multi_agent_pipeline_workflow(pipeline_id: str):
    ...
    queued_build = execute_pipeline(azure_devops_url, azure_devops_pat, azure_devops_project, pipeline_id, parameters=None)
    if queued_build:
...
def get_workflow_detail(workflow_id: str):
    ids = workflow_id
    json_data = get_workitem_details(azure_devops_url, azure_devops_pat, azure_devops_project,workitem_id= ids)
